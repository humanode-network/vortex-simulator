  ...
  return (
    <div className="app-page flex flex-col gap-6">
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Open cases", value: cases.filter((c) => c.status !== "closed").length },
          { label: "Jury panels", value: "12 seats / case" },
          { label: "New reports", value: "27 this week" },
          { label: "Closed cases (30d)", value: "6" },
        ].map((metric) => (
          <Card key={metric.label} className="border border-border bg-panel-alt text-center">
            <CardContent className="space-y-1 p-4">
              <p className="text-xs tracking-wide text-muted uppercase">{metric.label}</p>
      ...
      <Card className="border border-border bg-panel">
        ...
        <CardContent className="space-y-3">
          {cases.map((courtCase) => (
            <Card key={courtCase.id} className="border border-border bg-panel-alt">
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted">{courtCase.subject}</p>
                    <p className="text-lg font-semibold text-(--text)">{courtCase.title}</p>
                    <p className="text-xs text-muted">{courtCase.triggeredBy}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <Badge className={statusStyles[courtCase.status]} variant="outline">
                    ...
                    <p className="text-xs text-muted">Opened {courtCase.opened}</p>
                  </div>
                </div>
              </CardHeader>
